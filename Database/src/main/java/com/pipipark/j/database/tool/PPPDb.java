...
public class PPPDb {
	...
	public <M extends PPPDatabase> BasicDataSource dataSource(M db) {
		...
		if (ds == null) {
			...
			if (db.user() != null) {
				...
				ds.setPassword(PPPString.isEmpty(db.password())?"":db.password());
			}
	...
	public static <M extends PPPDatabase> Connection getConnection(M db)
			throws PPPSqlException {
		...
		if (conn == null) {
			...
			try {
				...
				throw new PPPSqlException("Connection can't be got Exception!",
						e);
			}
	...
	public static <M extends PPPDatabase> void closeConnection(M db)
			throws PPPSqlException {
		PPPDoubleMap<String, Class<?>, Connection> map = local.get();
		if (map == null) {
			return;
	...
	public <M extends PPPModel, N extends PPPDatabase> Class<M> model(
			String name, N db) throws PPPSqlException {
		List<Class<?>> list = ms.get(db.getClass());
		if (list == null) {
			throw new PPPSqlException("ModelClass: " + name
					+ " not found, you must be create it!");
		}
		for (Class<?> obj : list) {
			if (name.equals(PPPString.<M> className((Class<M>) obj)
					.toLowerCase())) {
				return (Class<M>) obj;
			}
		}
